...
class Polygon(Primitive):
    ...
    def __init__(self, color, points, pos):
        ...
        self.target_rect = pygame.Rect(self.min_x, self.min_y, self.max_x - self.min_x,
                                       self.max_y - self.min_y)  # Create a rectangle with the bounds
    ...
    def draw(self, display_surf):
        pygame.draw.polygon(self.shape_surf, self.color, [(x - self.min_x, y - self.min_y) for x, y in
                                                          self.points])  # Loop through coords and get new positions
